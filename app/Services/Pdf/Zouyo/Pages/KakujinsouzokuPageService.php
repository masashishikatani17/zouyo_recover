...
class KakujinsouzokuPageService implements ZouyoPdfPageInterface
{
    ...
    public function render(TCPDF $pdf, array $payload): void
    {
        ...
        */
        ...
        $wakusen = 0;
        $templatePath = resource_path('/views/pdf/07_pr_kakusisan.pdf');

        $pdf->setSourceFile($templatePath);
        $tpl = $pdf->importPage(1);
        $templateSize = $pdf->getTemplateSize($tpl);

        // テンプレートの実サイズ・向きでページを作成し、原点(0,0)に貼る
        $addTemplatePage = static function (TCPDF $pdf, $tpl, array $templateSize): void {
            $pdf->AddPage(
        ...
        $addTemplatePage($pdf, $tpl, $templateSize);
...
//dd($prefillFamily);
        ...
        */
        ...
        for ($no = 2; $no <= 10; $no++) {
            ...
            if ($bunsiInt !== null && $bunboInt !== null && $bunsiInt >= 1 && $bunboInt >= 1) {                $legalHeirCount++;
            }
        }
        ...
        for ($no = 1; $no <= 10; $no++) {
            ...
            /**
             * ▼ 2ページ目（相続人 no7〜10）用のレイアウト初期化
             *   - no=7 のタイミングで新しいページを追加し、同じテンプレートを敷いて座標をリセット
             */
            if ($no === 7) {
                ...
                // 被相続人氏名をPDFに描画
                $pdf->SetFont('mspgothic03', '', 12);
                $pdf->MultiCell(40, 10, $donorName , $wakusen, 'L', 0, 0, 45, 21.0);
            ...
            if ($no !== 1 && $hasHeirName) {
                ...
                $pdf->MultiCell(30, 10, $relLabel, $wakusen, 'C', 0, 0, $xx, $yy + 6.5);
            ...
            if ($no === 1 || $no === 7) {
                ...
                if ($basicDedKyen > 0) {
                    ...
                    // 基礎控除額の算式表示
                    // 固定文言は使わず、Controller / Calculator 側で
                    // マスターテーブルから組み立てた表示文言を payload 経由で受け取る
                    if ($basicDeductionFormulaLabel !== '') {
                        $pdf->SetFont('mspgothic03', '', 10);
                        $pdf->MultiCell(120, 10, $basicDeductionFormulaLabel, $wakusen, 'R', 0, 0, $xx - 130, $yy + 20.0);
                    }
                ...
                if ($taxableEstate > 0) {
                    $pdf->SetFont('mspgothic03', '', 12);
                    $pdf->MultiCell(28, 10, number_format($taxableEstate), $wakusen, 'R', 0, 0, $xx - 2.0 , $yy + 26.5);
                }
                ...
                if ($sozokuTaxTotalKyen > 0) {
                    $pdf->SetFont('mspgothic03', '', 12);
                    // 「相続税の総額」行の合計欄
                    $pdf->MultiCell(28, 10, number_format($sozokuTaxTotalKyen), $wakusen, 'R', 0, 0, $xx - 2.0, $yy + 40.3);
                }
                ...
                $pdf->MultiCell(28, 10, '1.0000', $wakusen, 'R', 0, 0, $xx - 2.0, $yy + 47.2);

                // ▼ 算出税額 合計（= heirs[].sanzutsu_tax_yen の合計 / 1000）
                if ($sumSanzutsuTotalKyen > 0) {
                    $pdf->MultiCell(28, 10, number_format($sumSanzutsuTotalKyen), $wakusen, 'R', 0, 0, $xx - 2.0, $yy + 53.2);
                }


                // ▼ ２割加算 合計（= Σ max(final_tax_yen − sanzutsu_tax_yen, 0) / 1000）
                if ($twoWarSumKyen > 0) {
                    $pdf->MultiCell(28, 10, number_format($twoWarSumKyen), $wakusen, 'R', 0, 0, $xx - 2.0, $yy + 59.2);
                }
                ...
                if ($totalGiftTaxCreditsKyen > 0) {
                    $pdf->MultiCell(28, 10, number_format($totalGiftTaxCreditsKyen), $wakusen, 'R', 0, 0, $xx - 2.0, $yy + 65.8);
                }
                ...
                if ($totalSpouseReliefKyen > 0) {
                    $pdf->MultiCell(28, 10, number_format($totalSpouseReliefKyen), $wakusen, 'R', 0, 0, $xx - 2.0, $yy + 72.4);
                }
                ...
                if ($totalOtherCreditsKyen > 0) {
                    $pdf->MultiCell(28, 10, number_format($totalOtherCreditsKyen), $wakusen, 'R', 0, 0, $xx - 2.0, $yy + 79.0);
                }
                ...
                if ($totalCreditsAllKyen > 0) {
                    $pdf->MultiCell(28, 10, number_format($totalCreditsAllKyen), $wakusen, 'R', 0, 0, $xx - 2.0, $yy + 85.6);
                }
                ...
                if ($totalSashihikiTaxKyen > 0) {
                    $pdf->MultiCell(28, 10, number_format($totalSashihikiTaxKyen), $wakusen, 'R', 0, 0, $xx - 2.0, $yy + 92.2);
                }
                ...
                if ($totalSettlementGiftKyen !== 0) {
                    $pdf->MultiCell(28, 10, number_format($totalSettlementGiftKyen), $wakusen, 'R', 0, 0, $xx - 2.0, $yy + 98.8);
                }
                ...
                if ($totalFinalAfterSettlementKyen > 0) {
                    $pdf->MultiCell(28, 10, number_format($totalFinalAfterSettlementKyen), $wakusen, 'R', 0, 0, $xx - 2.0, $yy + 105.4);
                }
                ...
                //   isanbunkatu.blade の「納付税額」行・合計欄に対応
                if ($sumPayableKyen > 0) {
                    $pdf->MultiCell(28, 10, number_format($sumPayableKyen), $wakusen, 'R', 0, 0, $xx - 2.0, $yy + 112.0);
                }
                ...
                //   isanbunkatu.blade の「還付税額」行・合計欄に対応
                if ($sumRefundKyen !== 0) {
                    $pdf->MultiCell(28, 10, number_format($sumRefundKyen), $wakusen, 'R', 0, 0, $xx - 2.0, $yy + 118.6);
                }
            ...
            if ($no !== 1 && $hasHeirName) {
                ...
                if ($heirTaxableEstateKyen > 0) {
                    $pdf->MultiCell(28, 10, number_format($heirTaxableEstateKyen), $wakusen, 'R', 0, 0, $xx, $yy + 26.5);
                }
...
//dd($family);
                ...
                $souzokunin = trim((string)$souzokunin);
                 ...
                 if ($lawShareVal !== ''
                     ...
                     $pdf->MultiCell(28, 10, $lawShareVal, $wakusen, 'C', 0, 0, $xx + 2.0, $yy + 33.4);
                 }
                ...
                */
                ...
                if ($heirSozokuTaxKyen > 0) {
                    $pdf->MultiCell(28, 10, number_format($heirSozokuTaxKyen), $wakusen, 'R', 0, 0, $xx, $yy + 40.3);
                }
                ...
                if ($heirAnbunRatio !== null && $heirTaxableTotalKyen > 0) {
                    ...
                    $pdf->MultiCell(28, 10, $ratioStr, $wakusen, 'R', 0, 0, $xx, $yy + 47.2);
                }
                ...
                if ($heirSanzutsuKyen > 0) {
                    $pdf->MultiCell(28, 10, number_format($heirSanzutsuKyen), $wakusen, 'R', 0, 0, $xx, $yy + 53.2);
                }
                ...
                if ($heirTwoWarKyen > 0) {
                    $pdf->MultiCell(28, 10, number_format($heirTwoWarKyen), $wakusen, 'R', 0, 0, $xx, $yy + 59.2);
                }
                ...
                if ($heirGiftCreditCalKyen > 0) {
                    $pdf->MultiCell(28, 10, number_format($heirGiftCreditCalKyen), $wakusen, 'R', 0, 0, $xx, $yy + 65.8);
                }
                ...
                if ($heirSpouseReliefKyen > 0) {
                    $pdf->MultiCell(28, 10, number_format($heirSpouseReliefKyen), $wakusen, 'R', 0, 0, $xx, $yy + 72.4);
                }
                ...
                if ($heirOtherCreditKyen > 0) {
                    $pdf->MultiCell(28, 10, number_format($heirOtherCreditKyen), $wakusen, 'R', 0, 0, $xx, $yy + 79.0);
                }
                ...
                if ($heirTotalCreditsKyen > 0) {
                    $pdf->MultiCell(28, 10, number_format($heirTotalCreditsKyen), $wakusen, 'R', 0, 0, $xx, $yy + 85.6);
                }
                ...
                if ($heirSashihikiKyen > 0) {
                    $pdf->MultiCell(28, 10, number_format($heirSashihikiKyen), $wakusen, 'R', 0, 0, $xx, $yy + 92.2);
                }
                ...
                if ($heirSettlementGiftKyen !== 0) {
                    $pdf->MultiCell(28, 10, number_format($heirSettlementGiftKyen), $wakusen, 'R', 0, 0, $xx, $yy + 98.8);
                }
                ...
                if ($heirPayableKyen > 0) {
                    $pdf->MultiCell(28, 10, number_format($heirPayableKyen), $wakusen, 'R', 0, 0, $xx, $yy + 112.0);
                }
                ...
                if ($heirRefundKyen > 0) {
                    $pdf->MultiCell(28, 10, number_format($heirRefundKyen), $wakusen, 'R', 0, 0, $xx, $yy + 118.6);
                }
     ...
     * 2. payload['summary']['basic_deduction_formula_label']
     * 3. summary 側に基礎控除の内訳があればそこから組み立て
     * 4. どれも無ければ空文字
     *
     * ※ PDFサービスではマスターテーブルを直接読まず、
     *    Controller / Calculator でマスターテーブルから取得した値を受けるだけにする。
     */
