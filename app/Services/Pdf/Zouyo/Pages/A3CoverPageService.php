 ...
 * 0: 表紙ページ
 */
class A3CoverPageService implements ZouyoPdfPageInterface
{
    ...
    public function render(TCPDF $pdf, array $payload): void
    {
        ...
        $templatePath = resource_path('/views/pdf/A3_00_hyoshi.pdf');
        ...
        if (preg_match('/^[(（]?\s*氏名未設定\s*[)）]?$/u', $customerName) === 1) {
            $customerName = '';
        }        
        ...
        */
        ...
        // 2. タイトル（中央）
        $pdf->SetFont('mspgothic03', '', 36);
        $pdf->SetTextColor(255,255,255);    //白
        $xx = 50;
        $yy = 97;
        $pdf->MultiCell(330, 10, $planTitle, $wakusen, 'C', 0, 0, $xx, $yy);
