...
<table class="table-base ms-10" style="width: 250px;">
  <tr>
    ...
    <td style="width: 130px;">
      <input type="text" class="form-control d-inline input-small"
             name="customer_name"
             style="width: 100px; background-color: #f0f0f0;"
      ...
      <table id="isan-left-table" class="table-compact-p">
        ...
        <tbody>
          ...
          <tr>
            ...
            <td>
              <input type="text" class="form-control suji8 comma decimal0"
                     style="background-color:#f0f0f0;" readonly tabindex="-1" value="1.0000">
            </td>
          </tr>
        ...
        <tbody>
          ...
          <tr>
            @for ($no = 2; $no <= 10; $no++)
              <td>
                <input type="text" class="form-control suji8 comma decimal0"
                       style="background-color:#f0f0f0;" readonly tabindex="-1"
                       data-role="taxable_total_heir" data-heir-no="{{ $no }}"
                       value="{{ number_format(
          ...
          <tr>
            @for ($no = 2; $no <= 10; $no++)
              @php
                ...
                // 法定相続分の表示は、title.blade.php で入力・保存している
                // 税法上の法定相続割合（bunsi / bunbo）を最優先で使う
                $bunsi = old(
                    "bunsi.$no",
          ...
          <tr>
            @for ($no = 2; $no <= 10; $no++)
              ...
              <td><input type="text" class="form-control suji8 comma decimal0" style="background-color:#f0f0f0;" readonly tabindex="-1" value="{{ isset($perHeirLegalTax) ? $toKyen($perHeirLegalTax) : '' }}"></td>
            @endfor
          </tr>
          ...
          <tr>
            @for ($no = 2; $no <= 10; $no++)
              ...
              <td><input type="text" class="form-control suji8 comma decimal0" style="background-color:#f0f0f0;" readonly tabindex="-1" value="{{ $toFixed4($ratio) }}"></td>
            @endfor
          </tr>

          <tr>
            @for ($no = 2; $no <= 10; $no++)
              <td><input type="text" class="form-control suji8 comma decimal0" style="background-color:#f0f0f0;" readonly tabindex="-1" value="{{ isset($heirsByIdx[$no]['sanzutsu_tax_yen']) ? $toKyen($heirsByIdx[$no]['sanzutsu_tax_yen']) : '' }}"></td>
            @endfor
          </tr>
          ...
          <tr>
            @for ($no = 2; $no <= 10; $no++)
              @php
                ...
                $inc = $inc > 0 ? $inc : 0;
              @endphp
              <td><input type="text" class="form-control suji8 comma decimal0" style="background-color:#f0f0f0;" readonly tabindex="-1" value="{{ $inc ? $toKyen($inc) : '' }}"></td>
            @endfor
          </tr>

          <tr>
            @for ($no = 2; $no <= 10; $no++)
              <td><input type="text" class="form-control suji8 comma decimal0" style="background-color:#f0f0f0;" readonly tabindex="-1" value="{{ isset($heirsByIdx[$no]['gift_tax_credit_calendar_yen']) ? $toKyen($heirsByIdx[$no]['gift_tax_credit_calendar_yen']) : '' }}"></td>
            @endfor
          </tr>

          <tr>
            @for ($no = 2; $no <= 10; $no++)
              <td><input type="text" class="form-control suji8 comma decimal0" style="background-color:#f0f0f0;" readonly tabindex="-1" value="{{ isset($heirsByIdx[$no]['spouse_relief_yen']) ? $toKyen($heirsByIdx[$no]['spouse_relief_yen']) : '' }}"></td>
            @endfor
          </tr>
          ...
          <tr>
            @for ($no = 2; $no <= 10; $no++)
              @php $hasHeirName = trim((string)($heirNames[$no] ?? '')) !== ''; @endphp
              <td>
                <input type="text" class="form-control suji8 comma decimal0 a"
          ...
          <tr>
            @for ($no = 2; $no <= 10; $no++)
              ...
              @endphp
              <td><input type="text" class="form-control suji8 comma decimal0" style="background-color:#f0f0f0;" readonly tabindex="-1" value="{{ $c_person ? $toKyen($c_person) : '' }}"></td>
            @endfor
          </tr>

          <tr>
            @for ($no = 2; $no <= 10; $no++)
              <td><input type="text" class="form-control suji8 comma decimal0" style="background-color:#f0f0f0;" readonly tabindex="-1" value="{{ isset($heirsByIdx[$no]['sashihiki_tax_yen']) ? $toKyen($heirsByIdx[$no]['sashihiki_tax_yen']) : '' }}"></td>
            @endfor
          </tr>

          <tr>
            @for ($no = 2; $no <= 10; $no++)
              <td><input type="text" class="form-control suji8 comma decimal0" style="background-color:#f0f0f0;" readonly tabindex="-1" value="{{ isset($heirsByIdx[$no]['settlement_gift_tax_yen']) ? $toKyen($heirsByIdx[$no]['settlement_gift_tax_yen']) : '' }}"></td>
            @endfor
          </tr>
          ...
          <tr>
            @for ($no = 2; $no <= 10; $no++)
              ...
              @endphp
              <td><input type="text" class="form-control suji8 comma decimal0" style="background-color:#f0f0f0;" readonly tabindex="-1" value="{{ $toKyen($rawSubtotalYen) }}"></td>
            @endfor
          </tr>
          ...
          <tr>
            @for ($no = 2; $no <= 10; $no++)
              ...
              @endphp
              <td><input type="text" class="form-control suji8 comma decimal0" style="background-color:#f0f0f0;" readonly tabindex="-1" value="{{ $toKyen($payableYen) }}"></td>
            @endfor
          </tr>
          ...
          <tr>
            @for ($no = 2; $no <= 10; $no++)
              ...
              @endphp
              <td><input type="text" class="form-control suji8 comma decimal0" style="background-color:#f0f0f0;" readonly tabindex="-1" value="{{ $toKyen($refundYen) }}"></td>
            @endfor
          </tr>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  //    1) まず現在の手入力値を autosave（ZouyoController@save）で DB に保存（非同期）
  //    2) 画面の切り替えは待たずに即座に行う（見た目は瞬時に切り替え）
  function autosaveManual() {
    ...
    fetch(form.action, {
      ...
      .then(function (resp) {
        ...
        // autosave は JSON / リダイレクト両方あり得るので、JSON でなくても無視してよい
        return resp.json().catch(function () { return {}; });
      })
      .catch(function (err) {
        console.error(err);
        // サイレントエラー（画面はすでに切り替えているのでアラートは出さない）
      });
  }
  ...
  modeRadios.forEach(function (r) {
    r.addEventListener('change', function () {
      ...
      if (nextMode === 'auto') {
        ...
        // 手入力 → 法定相続割合：
        // 1) 現在の手入力値を manualStore にスナップショット（念のため）
        // 2) 非同期で autosave（手入力モードとして保存）
        // 3) 即座に auto 表示へ切り替え（レスポンス待ちしない）
        if (currentMode === 'manual') {
          ...
          autosaveManual();          // fire-and-forget
        }
        applyInputMode('auto');
        ...
        // 法定相続割合 → 手入力：
        //  - 表示は manualStore に退避してある値（初期表示時のDB値＋手入力編集分）に切り替わる
