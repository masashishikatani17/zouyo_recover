...
@endphp
  ...
  <table class="table-base ms-0" style="width: 500px;">
    <tbody>
      <tr>
        ...
        <td class="text-start">
             <input type="text" class="form-control suji11"
      ...
      <tr class="border-b">
        <th>提案書の表題</th>
        <td class="text-start">
            <input type="text" class="form-control kana20"
                   name="header_title" style="ime-mode:active;"
                   value="{{ old('header_title', $prefillHeader['title'] ?? '') }}"
      ...
      <tr class="border-b align-middle">
        ...
        <td class="text-start">
          <div class="flex items-center gap-1">
            ...
            <input type="text" class="form-control suji4"
                   name="header_year" style="ime-mode:disabled;" inputmode="numeric"
                   value="{{ old('header_year', $prefillHeader['year'] ?? '') }}">
                   年

            <input type="text" class="form-control suji2"
                   name="header_month" style="ime-mode:disabled;" inputmode="numeric"
                   value="{{ old('header_month', $prefillHeader['month'] ?? '') }}">
                   月

            <input type="text" class="form-control suji2"
                   name="header_day" style="ime-mode:disabled;" inputmode="numeric"
                   value="{{ old('header_day', $prefillHeader['day'] ?? '') }}">
                   日
          </div>
        </td>
      </tr>
      <tr class="border-b">
        ...
        <td class="text-start">
            ...
            <input type="text" class="form-control kana30"
      ...
      <table class="table-compact-p ms-0 mb-3" style="width:144px;">
        ...
        <tbody>
          ...
          <tr style="height:25px;">
            ...
            <td>
              <input type="text" class="form-control kana8"
    ...
    <div class="family-right-wrap mt-2">
      <table class="table-compact-p ms-0 mb-3" style="table-layout: fixed;">
        ...
        <tbody>
          ...
          <tr>
            <td>
              <select class="form-control kana2" name="gender[1]">                
            ...
            <td>
              @php $rel = old('relationship.1', $prefillFamily[1]['relationship_code'] ?? ''); @endphp
              <select name="relationship[1]" class="form-control kana4">
            ...
            <td>
              @php $sz = old('souzokunin.1', $prefillFamily[1]['souzokunin'] ?? null); @endphp
              <select class="form-control kana7" name="souzokunin[1]">
            ...
            <td class="border px-1 py-0">
              <input type="text" class="form-control suji3"
          ...
          @for ($i = 2; $i <= 10; $i++)
            <tr>
              <td class="border px-1 py-0">
                <select class="form-control kana2 text-center" name="gender[{{ $i }}]">
              ...
              <td class="border px-1 py-0">
                ...
                <select name="relationship[{{ $i }}]" class="form-control kana4">
                  <option value="" @selected((string)$rel==='')></option>                
                  @foreach($relationships as $key => $label)
                    <option value="{{ $key }}" @selected((string)$rel===(string)$key)>{{ $label }}</option>
                  @endforeach
              ...
              <td class="border px-1 py-1">
                <input type="text" class="form-control kana4"
              ...
              <td class="border px-1 py-1">
                ...
                <select class="form-control kana7" name="souzokunin[{{ $i }}]">
                  <option value="" @selected($sz===null || $sz==='')></option>
                  <option value="被相続人"      @selected($sz==='被相続人')>被相続人</option>
                  <option value="法定相続人"    @selected($sz==='法定相続人')>法定相続人</option>
                  <option value="法定相続人以外" @selected($sz==='法定相続人以外')>法定相続人以外</option>
  ...
  <div class="mt-3 ms-3">
      ...
      ※金融資産の運用利回り(税引後)	
            <input type="text" class="form-control d-inline input-small text-end"
                   name="per" style="width:60px; ime-mode:disabled;" inputmode="numeric"
                   value="{{ old('per', $prefillHeader['per'] ?? '') }}">
      ％	<br>
      ※相続税の２割加算の対象になる人は「ﾚ」を付けて下さい。	<br>
      ※暦年課税の贈与で特例贈与の適用を受けられる受贈者は「ﾚ」を付けて下さい。年齢による判定は自動で行ないます。	<br>
      <span class="ms-4">法定相続人の人数
            <input type="text" class="form-control suji2" name="houtei_ninzu" readonly tabindex="-1">
      人	</span>
  </div>
  ...
  {{-- 氏名が空欄の時はその右側も空欄 --}}
      ...
      function toggleRowFields(i) {
        ...
        const selectors = [
          ...
          `input[name="bunsi[${i}]"]`,
          `input[name="bunbo[${i}]"]`,
          `input[name="twenty_percent_add[${i}]"]`,
          `input[name="tokurei_zouyo[${i}]"]`,
      ...
      function prepareRowFieldsForSubmit() {
        for (let i = 1; i <= 10; i++) {
          ...
          const selectors = [
            ...
            `input[name="bunsi[${i}]"]`,
            `input[name="bunbo[${i}]"]`,
            `input[name="twenty_percent_add[${i}]"][type="checkbox"]`,
            `input[name="tokurei_zouyo[${i}]"][type="checkbox"]`,
            `input[name="birth_year[${i}]"]`,
            `input[name="birth_month[${i}]"]`,
            `input[name="birth_day[${i}]"]`,
            `input[name="age[${i}]"]`,
